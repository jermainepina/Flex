...
1. **Goals tab** (`/goals`, migration `0007_goals.sql`): `goals` table (`metric` enum sessions/volume/exercise_weight/cardio_minutes, `period` enum weekly/monthly — null for standing exercise-weight targets, `target` numeric canonical count/kg/minutes, optional `exercise_id`, RLS owner-only). Pure progress math in `src/lib/goals.ts` (`computeGoalProgress`, `goalLabel` — fixture-tested via `scripts/verify-goals.mjs`); create/delete server actions; goal cards with `CircularStat` rings + achieved (lime ring) state. The dashboard's **main card** (full width, top of page) is the goal tracker: every goal stacked vertically with its progress ring + current/target; empty state is a full card linking to `/goals` ("Add a goal →"). Latest-PR info lives as a caption line in the Best lifts card. The training-days heatmap's week columns flex to fill the card width (large dots). **One-shot windows (migration `0008`)**: goals measure a definite window anchored at `created_at` — weekly goals choose `week_anchor` 'monday' (calendar week) or 'rolling' (7 days from creation; labels read "/ 7 days") at creation; monthly = the calendar month of creation; exercise-weight goals stay standing. `goalWindow`/`expired`/`missed` in `lib/goals.ts`. **Missed goals** (window ended, unachieved) show a red shake + "Goal missed" card state on the goals page, then auto-fade and self-delete (~2.3s total, `goal-card.tsx`); the dashboard tracker just filters them out. Achieved goals never auto-delete (user clears via ✓). **"Help me choose" wizard** (`goal-wizard.tsx`, with −/+ steppers per suggestion — manual tweaks reset whenever an answer changes — and a weekly-window pill applied to what it creates): three pill questions (frequency / objective cut-bulk-maintain-strength-fitness / cardio appetite) → deterministic rule-based suggestions in `suggestGoals` (`lib/goals.ts`, fixture-tested; volume targets anchored to the user's real last-4-weeks average and rounded to clean display-unit steps, skipped entirely with no history — **no AI**, per the 2026-07-13 descoping); approved via checkboxes → `createGoals` bulk action. **Completed-goal check-off** (`goal-card.tsx`): achieved cards swap Remove for a lime ✓ button that fades the card out (~500ms scale/opacity) then deletes. **Completion celebration**: when a saved workout (lifting or cardio) pushes a goal over its target, a full-screen overlay fires first (confetti + "GOAL CRUSHED" + goal labels crossed off with staggered strike-through, `goal-celebration.tsx`, `confetti-fall`/`goal-strike` keyframes) with a Continue button onward to the summary/detail page; detection = `newlyCompletedGoals` in `lib/goals.ts` (progress with vs without the new workout's rows, computed fail-soft in `findCompletedGoals` inside `log/actions.ts` — already-achieved goals never re-fire).
